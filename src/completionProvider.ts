...
export class RotationCompletionProvider implements vscode.CompletionItemProvider {
    ...
    private getCycleCompletions(partial: string): vscode.CompletionItem[] {
        const cycleProps = [
            ...
            { name: 'tank', desc: 'Current cycle member role is tank' },
            { name: 'healer', desc: 'Current cycle member role is healer' },
            { name: 'dps', desc: 'Current cycle member role is DPS' },
            { name: 'time_to_die', desc: 'Estimated seconds until cycle member dies' },
